...
- **ECC**: `Get Screen Signature` (mixin `keywords/_perception.py`) gives a text view of
...
- **Fiori**: `Get Ui5 Page Tree` returns the UI5 control hierarchy serialized to XML (tag =
...
- `fiori_through_rfmcp.py` scores **8/8** vs the project's local UI5 fixture: plugin
  discovery, two-library session, routing, populated UI5 tree, click opens a dialog.
- `ecc_through_rfmcp.py` scores **9/9** vs a live ABAP Platform A4H (Docker): COM login,
