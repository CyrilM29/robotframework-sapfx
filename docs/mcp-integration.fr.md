...
provider que pour la *lib d'automatisation web active*, résolue à **Browser** — donc le
provider Browser l'emporte et celui de Fiori est court-circuité pour la page source.
...
- `ecc_through_rfmcp.py` — **9/9** sur un ABAP Platform A4H live (Docker) : login COM,
  routing, `Get Screen Signature`, et le **state provider** renvoyant la vraie
