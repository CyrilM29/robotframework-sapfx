...
via MCP), et relie chaque point aux **keywords de préflight** qui le vérifient
automatiquement.
...
- Compte Windows dédié à l'exécution des tests ; les répertoires de captures
  (`tools/recorder/captures/`, `results/` Robot) restent hors VCS, car les
...
- rf-mcp tourne en **stdio** (aucune écoute réseau) ; le garder ainsi, sans
...
- Le pack de déploiement (le seul canal d'installation complète : recorders,
  plugins rf-mcp, agents compris) est distribué via les GitHub Releases du
  dépôt public ; PyPI ne livre que les bibliothèques. Vérifier qu'on installe
  les wheels d'une release de confiance (sidecar SHA-256, SBOM, provenance),
  pas d'un fork.
