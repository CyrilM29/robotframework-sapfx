...
| `sapgui/user_scripting_set_readonly` | `FALSE` on test systems | when `TRUE` the API can read but not act, and tests fail strangely |
...
- Dedicated Windows account for test execution; captures/screenshots
  ...
  as screenshots can contain personal data (GDPR): treat run artifacts as data.
...
- rf-mcp runs in **stdio** (no network listener); keep it that way and do not
...
- Prompt-injection containment: an agent acts with the test user's rights, so
...
- The deployment pack (the only full-install channel: recorders, rf-mcp
  plugins, agents included) is distributed via the public repo's GitHub
  Releases; PyPI carries the libraries only. Verify you install wheels from a
  release you trust (SHA-256 sidecar, SBOM, provenance), not a fork.
