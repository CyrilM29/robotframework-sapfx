...
#
# This module is a near-verbatim copy of `SapGuiLibrary.py` from
#   robotframework-sapguilibrary
#   (https://github.com/frankvanderkuur/robotframework-sapguilibrary)
#   Copyright (c) Frank van der Kuur — Licensed under the Apache License 2.0
#
# Local modification (kept minimal so upstream can be re-synced):
#   * Class renamed `SapGuiLibrary` -> `SapGuiBase` so SapEccLibrary can
#     subclass it. All keyword behaviour is unchanged.
#
# New / overriding behaviour lives in `SapEccLibrary/keywords/` mixins and in
# `SapEccLibrary.py`. See ../../NOTICE and ../../docs/audit-upstream.md.
# -----------------------------------------------------------------------------
import pythoncom
