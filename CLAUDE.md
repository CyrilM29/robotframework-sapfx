...
| `src/sapfx_common/gateway_status.py` | **Préflight du canal API**, logique pure : classification d'une sonde du catalogue Gateway (`ok`/`unreachable`/`auth_failed`/`forbidden`/`catalog_not_found`/`gateway_inactive`/`server_error`), chaque état avec sa remédiation NOMMÉE ; le cas fondateur vécu (conteneur A4H re-créé → HTTP 500 `/IWFND/CM_COS/003`) pointe l'activité IMG `/IWFND/IWF_ACTIVATE` (docs/ecc-validation.md §11.7). Socle de `Get Gateway Status`/`Gateway Should Be Active`/`Wait Until Api Available`. Typed. |
...
| `src/sapfx_common/rfc_tables.py` | **`RFC_READ_TABLE`** pur : construction des paramètres (clauses OPTIONS ≤ 72 caractères, `abap_quote`), parsing des lignes `DATA` par délimiteur, et la classification des statuts de job **TBTCO** (`job_wait_verdict` : done/aborted/waiting/missing). Socle de `Wait For Background Job`. Typed. |
