...
| `src/SapEccLibrary/keywords/_grid.py` | Mixin: ALV grid by column title, `Read Grid`/`Read Full Grid` (argument `columns` : lecture RESTREINTE à des ids TECHNIQUES, clés indépendantes de la locale ET du profil d'affichage ALV, beaucoup moins d'appels COM sur une grille large), adressage de ligne par contenu (`Get Cell Value By Row Content`), et `Read Abap List` (sorties liste classiques sans objet grille : reconstruction géométrique via `sapfx_common.abap_list`, mode accessibilité SAP GUI conseillé). Aussi les **GuiTableControl** (tables de dynpro classiques : FB60, VA01 ancienne génération) par titre de colonne : `Read Table Control` (défilement par fenêtres, objet COM ré-acquis après chaque scroll, colonnes sans titre `COL<n>`, doublons suffixés), `Get/Set Table Control Cell` (ligne ABSOLUE, scroll automatique, cellule non modifiable = échec actionnable), `Find Table Control Row` ; logique pure dans `sapfx_common.table_control`. |
...
| `src/SapFioriLibrary/_ui5_js.py` | Injected `__SAPFX` JS bundle: control tree, XPath + role engines, shortest-xpath, matcher, capture, WebGUI sid (ported from playwright-sap), generic DOM engine (`resolveByDom` + `captureDom` pour le recorder), page-composition probe (`pageComposition`). Also generates the Recorder snippet. Le JS vit dans `_ui5_bundle.js.tpl` / `_ui5_spy_listener.js.tpl` (vrais fichiers JS, embarqués dans le wheel) ; le module Python les charge et les assemble. |
...
- [x] **Release 0.6.6, le canal API devient une plateforme de données
      ...
      **Validation live** vs A4H (API 16/16, ECC 8/8) et cap-sflight (3/3),
      ...
      ubuntu : 93 %).
- [x] **0.6.7, revue de `integrations/` et de ses dépendances (2026-08-19,
      version PRÉPARÉE, publication non faite)** : 24 constats traités, dont
      ...
      **Second lot du même jour, recette complète puis fermeture d'un angle
      ...
      n'avait **pas de suite à lui** (33 keywords publics éprouvés en live
      ...
      Reste à faire pour publier : tag, dépôt public, PyPI.
